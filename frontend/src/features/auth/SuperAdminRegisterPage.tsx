...
export default function SuperAdminRegisterPage({ onBack, onLoginInstead, onSuccess }: Props) {
  ...
  // Ref prevents double-submit from StrictMode or double-click
  ...
  const validate = (): boolean => {
    ...
    if (!form.phone.trim())       e.phone     = 'Phone number is required';
      ...
      const saLocal = /^0[6-8][0-9]{8}$/.test(digits);
      const saIntl  = /^\+27[6-8][0-9]{8}$/.test(digits);
      if (!saLocal && !saIntl) e.phone = 'Enter a valid SA number (e.g. 0821234567)';
    }
  ...
  const handleSubmit = async () => {
    // Guard: ignore if already submitted (handles StrictMode double-invoke & double-click)
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sar-page { font-family:'DM Sans',sans-serif; min-height:100vh; width:100%; background:#071a0d; display:flex; flex-direction:column; padding:1.5rem 2rem 2.5rem; }
        ...
        .sar-input.err { border-color:#f87171; }
        .sar-input::placeholder { color:rgba(255,255,255,0.2); }
        .sar-error { font-size:0.72rem; color:#f87171; margin-top:0.28rem; font-weight:500; }
        ...
        .btn-gold { width:100%; background:#f0c000; color:#071a0d; border:none; padding:0.85rem; border-radius:9px; font-size:0.95rem; font-weight:700; cursor:pointer; font-family:'DM Sans',sans-serif; transition:all 0.2s; display:flex; align-items:center; justify-content:center; gap:0.5rem; }
        .btn-gold:hover:not(:disabled) { background:#ffd200; transform:translateY(-1px); box-shadow:0 6px 20px rgba(240,192,0,0.35); }
        ...
        @media (max-width:880px) { .sar-form-body { grid-template-columns:1fr; } .sar-col:first-child { border-right:none; border-bottom:1px solid rgba(255,255,255,0.06); } .sar-page { padding:1rem 1.25rem 2rem; } }
        @media (max-width:540px) { .sar-row2 { grid-template-columns:1fr; } }
      `}</style>
        ...
        <div className="sar-form-body">
          <div className="sar-col">
            <div className="sar-info-box">
              <span>🔒</span>
              <span>Super Admin accounts have <strong style={{ color: '#f0c000' }}>full system access</strong>.</span>
            </div>
            ...
            <div className="sar-field">
              ...
              {!errors.phone && <span style={{ fontSize:'0.65rem', color:'rgba(255,255,255,0.25)', marginTop:'0.2rem' }}>SA number · 10 digits starting with 0 (e.g. 0821234567)</span>}
            </div>
          ...
          <div className="sar-col">
            {selectedPos ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'rgba(240,192,0,0.07)', border: '1px solid rgba(240,192,0,0.2)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1.25rem' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1.25rem', fontSize: '0.8rem', color: 'rgba(255,255,255,0.25)', fontStyle: 'italic' }}>
