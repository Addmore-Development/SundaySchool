...
function ConsentModal({ child, onClose }: { child: Child; onClose: () => void }) {
  ...
  const handleSave = () => {
    console.log('Consent saved:', { activities, medical, photos, camps });
    setSaved(true);
        ...
        <div style={{ padding: '1.5rem' }}>
          {items.map(({ label, value, set: setter }) => (
            ...
            >
              <div style={{
                ...
                border: `2px solid ${value ? '#34d399' : 'rgba(255,255,255,0.15)'}`,
          ...
          <button
            ...
            style={{
              ...
              border: 'none', padding: '0.8rem', borderRadius: '9px',
...
export default function ParentDashboard({
  ...
  const getChildName = (id: string) => {
    const c = MOCK_CHILDREN.find(ch => ch.id === id);
              ...
              <div className="pd-fade">
                ...
                <div className="pd-hero">
                  ...
                  <p className="pd-hero-body">
                    You have <strong style={{ color: '#fff' }}>{MOCK_CHILDREN.length} children</strong> registered.
                    {unread > 0 && (
                  ...
                  <div className="pd-hero-chips">
                    {[
                      ...
                      { l: 'Children', v: `${MOCK_CHILDREN.length} registered` },
                    ].map(({ l, v }) => (
                ...
                <div className="pd-stats-grid">
                  {[
                    { label: 'Children Registered', value: MOCK_CHILDREN.length, color: '#34d399', icon: '👶' },
                    { label: 'Avg Attendance', value: `${Math.round(MOCK_CHILDREN.reduce((s, c) => s + c.attendanceRate, 0) / MOCK_CHILDREN.length)}%`, color: '#f0c000', icon: '📅' },
                    { label: 'Welfare Flags', value: MOCK_CHILDREN.reduce((s, c) => s + c.welfareFlags, 0), color: MOCK_CHILDREN.some(c => c.welfareFlags > 0) ? '#f87171' : '#34d399', icon: '🚩' },
                ...
                <div className="pd-card">
                  ...
                  <div className="pd-child-list">
                    {MOCK_CHILDREN.map(child => {
                      ...
                      return (
                        ...
                        >
                          <div className="pd-avatar" style={{ background: avatarColors[child.id] }}>
                  ...
                  <button onClick={onRegisterChild} className="pd-register-btn">
            ...
            {activeTab === 'children' && (
              <div className="pd-fade">
                <div className="pd-page-header">
                  ...
                  <button onClick={onRegisterChild} className="btn-green">+ Register Child</button>
                </div>
                  ...
                  <div>
                    ...
                    <div className="pd-card">
                      <div className="pd-detail-hero">
                        <div className="pd-avatar pd-avatar--lg" style={{ background: avatarColors[selectedChild.id] }}>
                      ...
                      return (
                        <div key={child.id} className="pd-card pd-child-card" onClick={() => setSelectedChild(child)}>
                          <div className="pd-child-card-top">
                            <div className="pd-avatar pd-avatar--md" style={{ background: avatarColors[child.id] }}>
                              {initials(child.firstName, child.lastName)}
                            </div>
...
.pd-user-avatar {
  ...
  display: flex; align-items: center; justify-content: center;
  font-size: 12px; font-weight: 700; color: #042a15;
}
...
/* ── Content — uncapped width, fills everything ── */
...
.pd-hero-pill {
  ...
  border-radius: 100px; background: rgba(52,211,153,0.12);
  border: 1px solid rgba(52,211,153,0.3); color: #34d399;
  font-size: 11.5px; font-weight: 700; font-family: 'Bebas Neue', sans-serif;
...
.pd-link-btn {
  background: none; border: none; color: rgba(255,255,255,0.4); font-size: 0.78rem;
  font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: color 0.15s; padding: 0;
}
...
/* ── Notification rows (overview) ── */
.pd-notif-row {
  border-radius: 9px; padding: 0.8rem 1rem; margin-bottom: 0.6rem;
...
/* ── Buttons ── */
.btn-green {
  background: #34d399; color: #042a15; border: none; padding: 0.6rem 1.25rem;
...
.pd-ghost-btn {
  ...
  color: rgba(255,255,255,0.55); padding: 0.45rem 1rem; border-radius: 7px;
...
.pd-action-btn {
  ...
  color: rgba(255,255,255,0.6); padding: 0.35rem 0.8rem; border-radius: 6px;
...
.pd-back-btn {
  ...
  cursor: pointer; margin-bottom: 1rem; padding: 0; transition: color 0.15s;
}
...
.pd-filter-btn {
  padding: 0.4rem 0.9rem; border-radius: 20px; background: rgba(255,255,255,0.04);
...
@media (max-width: 1024px) {
  .pd-children-grid { grid-template-columns: repeat(2, 1fr); }
}
...
@media (max-width: 640px) {
  .pd-header        { height: auto; padding: 10px 16px; }
  .pd-user-chip     { display: none; }
  .pd-tab-nav       { padding: 0 8px; }
  .pd-tab           { padding: 0 10px; font-size: 0.78rem; }
  .pd-stats-grid    { grid-template-columns: repeat(2, 1fr); gap: 0.65rem; }
  .pd-info-grid     { grid-template-columns: 1fr 1fr; }
  .pd-hero          { padding: 18px; }
  .pd-main          { padding: 14px; }
  .pd-detail-hero   { flex-direction: column; }
  .pd-att-date      { min-width: 70px; }
}

@media (max-width: 400px) {
  .pd-signout       { display: none; }
  .pd-stats-grid    { grid-template-columns: 1fr 1fr; gap: 0.5rem; }
  .pd-stat-card     { padding: 0.75rem 0.5rem; }
  .pd-stat-value    { font-size: 1.5rem; }
  .pd-info-grid     { grid-template-columns: 1fr; }
  .pd-hero-heading  { font-size: 20px; }
}
`;